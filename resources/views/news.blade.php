            ...
            <div class="swiper-wrapper">
                ...
                @foreach ($slideshows as $slide)
                    <div class="swiper-slide relative">
                        <img src="{{ $slide->gambar_url }}" alt="Slide {{ $loop->iteration }}"
                    ...
                    @foreach ($beritas as $berita)
                        ...
                        <div class="animate_top sg vk rm xm">
                            <div class="c rc i z-1 pg">
                                <img class="w-full h-60" src="{{ $berita->gambar1_url }}" alt="{{ $berita->judul }}" />

                                <div class="im h r s df vd yc wg tc wf xf al hh/20 nl il z-10">
                                    <a href="/news/{{ $berita->slug }}"
                            ...
                            <div class="yh">
                                ...
                                <p class="mt-2 text-gray-600">
                                    {{ \Illuminate\Support\Str::words(strip_tags($berita->excerpt), 10, '...') }}
                                </p>
                            </div>
                        </div>
                    @endforeach
                        ...
                        <nav>
                            <ul class="tc wf xf bg">
                                ...
                                @if ($beritas->onFirstPage())
                                    <li>
                                        <span
                                            class="c tc wf xf wd in zc hn rg uj fo wk xm ml il hh rm tl zm yl an opacity-50 cursor-not-allowed">
                                ...
                                @foreach ($beritas->getUrlRange(1, $beritas->lastPage()) as $page => $url)
                                    @if ($page == $beritas->currentPage())
                                        <li>
                                            <span
                                                class="c tc wf xf wd in zc hn rg uj fo wk xm ml il hh rm tl zm yl an bg-blue-500 text-white">
                                                {{ $page }}
                                            </span>
                                        </li>
                                    @else
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const swiper = new Swiper(".mySwiper", {
            ...
            speed: 800,
            autoplay: {
                delay: 10000, // 10 detik seperti yang diminta
                disableOnInteraction: false,
                pauseOnMouseEnter: false, // Biarkan terus berjalan meski hover
            },
            pagination: {
                el: ".swiper-pagination",
                clickable: true,
            ...
            mousewheel: false,
            effect: 'slide',
            // Tambahkan ini untuk memastikan autoplay berjalan
            on: {
                init: function() {
        ...
        // Force autoplay start (jika masih bermasalah)
        setTimeout(() => {
