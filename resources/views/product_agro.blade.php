    ...
    <div class="container mx-auto px-6 lg:px-8 max-w-7xl">
        @if (!$agro)
            <div class="text-center text-gray-500 py-20">
                <h2 class="text-3xl font-semibold mb-4">Belum ada data Agro</h2>
            ...
            <div class="text-center mb-16">
                ...
                @if ($agro->gambar)
                    <div class="mt-12 relative max-w-5xl mx-auto">
                        ...
                        <img src="{{ asset('storage/' . $agro->gambar) }}" alt="Gambar Utama"
                ...
                @foreach ([1, 2, 3] as $i)
                    ...
                    @if (!empty($agro->$nama))
                        <div
                            ...
                            @if (!empty($agro->$gambar))
                                <div class="overflow-hidden">
                                    <img src="{{ asset('storage/' . $agro->$gambar) }}" alt="{{ $agro->$nama }}"
                                        class="w-full h-64 object-cover transform group-hover:scale-105 transition-transform duration-500">
                                </div>
                            @endif
                            <div class="p-6 text-center">
                                <h3 class="font-bold text-2xl text-green-700 mb-2">{{ $agro->$nama }}</h3>
                                <p class="text-gray-600 leading-relaxed text-base">{{ $agro->$desk }}</p>
                            </div>
                        </div>
                    @endif
                @endforeach
