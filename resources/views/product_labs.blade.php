...
<section class="relative pt-32 pb-20 bg-gradient-to-b from-green-50 to-white overflow-hidden">
    <div class="contlabsner mx-auto px-6 lg:px-8 max-w-7xl">
        @if (!$labs)
            <div class="text-center text-gray-500 py-20">
                <h2 class="text-3xl font-semibold mb-4">Belum ada data labs</h2>
            ...
            <div class="text-center mb-16">
                ...
                @if ($labs->gambar)
                    <div class="mt-12 relative max-w-5xl mx-auto">
                        ...
                        <img src="{{ asset('storage/' . $labs->gambar) }}" alt="Gambar Utama"
                ...
                @foreach ([1, 2, 3] as $i)
                    ...
                    @if (!empty($labs->$nama))
                        <div
                            ...
                            @if (!empty($labs->$gambar))
                                <div class="overflow-hidden">
                                    <img src="{{ asset('storage/' . $labs->$gambar) }}" alt="{{ $labs->$nama }}"
                                        class="w-full h-64 object-cover transform group-hover:scale-105 transition-transform duration-500">
                                </div>
                            @endif
                            <div class="p-6 text-center">
                                <h3 class="font-bold text-2xl text-green-700 mb-2">{{ $labs->$nama }}</h3>
                                <p class="text-gray-600 leading-relaxed text-base">{{ $labs->$desk }}</p>
                            </div>
                        </div>
                    @endif
                @endforeach
...
@include('components.modal-buku-tamu', ['produk' => 'Smartgarden labs'])
